...
import 'package:simpulx/features/chat/presentation/pages/web_file_helpers.dart'
    as web_helpers;
import 'package:dio/dio.dart' show DioMediaType, FormData, MultipartFile, Options, ResponseType;
...
class _ChatDetailPageState extends State<ChatDetailPage> {
  ...
  Future<void> _startRecording() async {
    try {
      ...
      if (!hasPermission) {
        if (mounted) {
          AppSnackbar.error(context, 'Microphone access is required. Please allow microphone permission in your browser/device settings.');
        }
        return;
      }
    ...
    if (text.startsWith('/') && text.length > 1 && !text.contains(' ')) {
      _ensureQuickReplyCache().then((_) {
        ...
        final matches = _quickReplyCache.where((r) {
          ...
          final sc = shortcut.startsWith('/') ? shortcut.substring(1) : shortcut;
  ...
  Future<void> _showTemplatePicker() async {
    ...
    final matches = cubitState.conversations.where((c) => c.id == widget.conversationId);
    if (matches.isEmpty) return;
    final conversation = matches.first;
    final channelId = conversation.whatsappChannelId;
    if (channelId == null || channelId.isEmpty) {
      AppSnackbar.error(context, 'No WhatsApp channel linked to this conversation');
      return;
    }
            ...
            return Container(
              ...
              child: ListView.separated(
                ...
                itemBuilder: (_, i) {
                  ...
                  return ListTile(
                    dense: true,
                    leading: Icon(Icons.bolt_rounded, size: 18,
  ...
  Widget _buildAttachmentPreview(ThemeData theme) {
    ...
    final sizeLabel = attachment.bytes.length > 1024 * 1024
        ? '$sizeMb MB'
        : '$sizeKb KB';
...
// ── Header Stage Badge ─────────────────────────────────
class _HeaderStageBadge extends StatelessWidget {
  final String name;
  final String? hexColor;

  const _HeaderStageBadge({required this.name, this.hexColor});

  @override
  Widget build(BuildContext context) {
    final c = _parseHex(hexColor) ?? const Color(0xFF6B7280);
    return Container(
...
class _SnoozeDialogState extends State<_SnoozeDialog> {
  ...
  Widget build(BuildContext context) {
    ...
    return AlertDialog(
      ...
      title: Row(
        children: [
          Icon(Icons.snooze_rounded, color: const Color(0xFFD88222), size: 22),
          const SizedBox(width: 10),
          const Text('Snooze Conversation'),
        ],
      ),
      content: Column(
        mainAxisSize: MainAxisSize.min,
...
class _MoreActionsMenu extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return PopupMenuButton<String>(
      ...
      itemBuilder: (context) => [
        _menuItem(context, 'quick_reply', Icons.bolt_outlined, 'Quick Reply', theme),
        _menuItem(context, 'template', Icons.description_outlined, 'Template', theme),
      ],
    );
  }
            ...
            child: Row(
              children: [
                ...
                TextButton.icon(
                  ...
                  icon: Icon(_showCreateForm ? Icons.list_rounded : Icons.add, size: 18),
    ...
    try {
      ...
      setState(() {
        _stages = filter == null
            ? all
            : all
                .where((s) => filter.contains(
                    (s['category'] as String?) ?? 'progressing'))
                .toList();
        _loading = false;
      });
    } catch (_) {
