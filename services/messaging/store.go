...
func (s *store) insertOutbound(ctx context.Context, orgID, convID, senderType, senderID, msgType, body, mediaURL, externalID, status string, previewText string, metadataJSON string) (string, error) {
	...
	err = tx.QueryRow(ctx,
		// metadata is NOT NULL (defaults to '{}'), so an absent payload must fall back
		// to an empty object — inserting NULL violated the constraint and failed every
		// outbound insert. $10 is also cast to text explicitly, since Postgres cannot
		// infer a parameter's type inside NULLIF($10,'') where both sides are unknown.
