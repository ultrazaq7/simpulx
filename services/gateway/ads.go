...
func (s *server) handleAdPerformance(w http.ResponseWriter, r *http.Request) {
	...
	rlq := `SELECT cv.id::text AS conversation_id, ct.id::text AS contact_id, cv.created_at, ct.full_name AS contact_name, ct.phone AS contact_phone, ct.email AS contact_email, cv.channel, cv.interest_level, ` + sourceClassifyExpr("cv") + ` AS source, st.name AS stage
