...
func main() {
	...
	// ── Auth ──
	mux.HandleFunc("POST /api/public/account-deletion", authRL(s.handleAccountDeletionRequest))
	...
	mux.HandleFunc("POST /api/calls/{id}/accept", s.requireAuth(s.callScoped(s.handleAcceptCall)))
	mux.HandleFunc("POST /api/calls/{id}/reject", s.requireAuth(s.callScoped(s.handleRejectCall)))
	mux.HandleFunc("POST /api/calls/{id}/connected", s.requireAuth(s.callScoped(s.handleCallConnected)))
	mux.HandleFunc("POST /api/calls/{id}/end", s.requireAuth(s.callScoped(s.handleEndCall)))
	mux.HandleFunc("GET /api/calls/{id}", s.requireAuth(s.callScoped(s.handleGetCall)))
	mux.HandleFunc("POST /api/calls/{id}/recording", s.requireAuth(s.callScoped(s.handleSaveCallRecording)))
	...
	mux.HandleFunc("POST /api/stages", s.requireAuth(s.gate("manage_pipeline_stages", s.handleCreateStage)))
	mux.HandleFunc("POST /api/stages/reorder", s.requireAuth(s.gate("manage_pipeline_stages", s.handleReorderStages)))
	mux.HandleFunc("PATCH /api/stages/{id}", s.requireAuth(s.gate("manage_pipeline_stages", s.handleUpdateStage)))
	mux.HandleFunc("DELETE /api/stages/{id}", s.requireAuth(s.gate("manage_pipeline_stages", s.handleDeleteStage)))
	...
	mux.HandleFunc("POST /api/conversations/{id}/assign", s.requireAuth(s.gate("assign_chats", s.handleAssign)))
	mux.HandleFunc("POST /api/conversations/{id}/campaign", s.requireAuth(s.gate("assign_chats", s.handleSetConversationCampaign)))
	...
	mux.HandleFunc("POST /api/conversations/{id}/close", s.requireAuth(s.gate("close_chats", s.handleClose)))
	...
	mux.HandleFunc("POST /api/knowledge", s.requireAuth(s.gate("manage_ai", s.handleAddKnowledge)))
	mux.HandleFunc("GET /api/ai-agent", s.requireAuth(s.handleGetAIAgent))
	mux.HandleFunc("PUT /api/ai-agent", s.requireAuth(s.gate("manage_ai", s.handleUpdateAIAgent)))
	...
	mux.HandleFunc("POST /api/custom-fields", s.requireAuth(s.gate("manage_custom_fields", s.handleCreateCustomField)))
	mux.HandleFunc("PATCH /api/custom-fields/{id}", s.requireAuth(s.gate("manage_custom_fields", s.handleUpdateCustomField)))
	mux.HandleFunc("DELETE /api/custom-fields/{id}", s.requireAuth(s.gate("manage_custom_fields", s.handleDeleteCustomField)))
	...
	mux.HandleFunc("GET /api/leads", s.requireAuth(s.gate("view_contacts", s.handleListLeads)))
	...
	mux.HandleFunc("POST /api/contacts/bulk-update", s.requireAuth(s.gate("edit_contacts", s.handleBulkUpdateContacts)))
	...
	mux.HandleFunc("GET /api/campaigns/{id}/ads/{adId}/preview", s.requireAuth(s.campaignScoped(s.handleAdPreviewHTML)))
	...
	mux.HandleFunc("GET /api/campaigns/{id}/ads/pages", s.requireAuth(s.gate("manage_campaigns", s.campaignScoped(s.handleListAdPages))))
	...
	mux.HandleFunc("POST /api/auth/meta-ads/connect", s.requireAuth(s.gate("manage_channels", s.handleMetaAdsConnect)))
	mux.HandleFunc("GET /api/auth/meta-ads/callback", s.handleMetaAdsCallback)
	mux.HandleFunc("POST /api/auth/tiktok-ads/connect", s.requireAuth(s.gate("manage_channels", s.handleTikTokAdsConnect)))
	mux.HandleFunc("GET /api/auth/tiktok-ads/callback", s.handleTikTokAdsCallback)
	...
	mux.HandleFunc("POST /api/templates", s.requireAuth(s.gate("manage_templates", s.handleCreateTemplate)))
	mux.HandleFunc("PATCH /api/templates/{id}", s.requireAuth(s.gate("manage_templates", s.handleUpdateTemplate)))
	mux.HandleFunc("DELETE /api/templates/{id}", s.requireAuth(s.gate("manage_templates", s.handleDeleteTemplate)))
	mux.HandleFunc("POST /api/templates/{id}/submit", s.requireAuth(s.gate("manage_templates", s.handleSubmitTemplate)))
	...
	mux.HandleFunc("GET /api/system-logs/messages", s.requireAuth(s.gate("menu_audit_log", s.handleLogMessages)))
	mux.HandleFunc("GET /api/system-logs/conversations", s.requireAuth(s.gate("menu_audit_log", s.handleLogConversations)))
	mux.HandleFunc("GET /api/system-logs/conversations/{id}/export", s.requireAuth(s.gate("menu_audit_log", s.handleExportConversation)))
	mux.HandleFunc("GET /api/system-logs/calls", s.requireAuth(s.gate("menu_audit_log", s.handleLogCalls)))
	mux.HandleFunc("GET /api/system-logs/activity", s.requireAuth(s.gate("menu_audit_log", s.handleLogActivity)))
	mux.HandleFunc("POST /api/exports", s.requireAuth(s.gate("menu_audit_log", s.handleCreateExport)))
	mux.HandleFunc("GET /api/exports", s.requireAuth(s.gate("menu_audit_log", s.handleListExports)))
	mux.HandleFunc("GET /api/organization", s.requireAuth(s.handleGetOrganization))
	mux.HandleFunc("PATCH /api/organization", s.requireAuth(s.gate("manage_organization", s.handleUpdateOrganization)))
	mux.HandleFunc("GET /api/web-api-sources", s.requireAuth(s.handleListWebAPISources))
	mux.HandleFunc("POST /api/web-api-sources", s.requireAuth(s.gate("manage_channels", s.handleCreateWebAPISource)))
	mux.HandleFunc("PATCH /api/web-api-sources/{id}", s.requireAuth(s.gate("manage_channels", s.handleUpdateWebAPISource)))
	mux.HandleFunc("POST /api/web-api-sources/{id}/regenerate-key", s.requireAuth(s.gate("manage_channels", s.handleRegenerateWebAPIKey)))
	mux.HandleFunc("DELETE /api/web-api-sources/{id}", s.requireAuth(s.gate("manage_channels", s.handleDeleteWebAPISource)))
	...
	mux.HandleFunc("GET /api/campaigns/{id}", s.requireAuth(s.campaignScoped(s.handleGetCampaign)))
	mux.HandleFunc("PATCH /api/campaigns/{id}", s.requireAuth(s.campaignScoped(s.gate("manage_campaigns", s.handleUpdateCampaign))))
	mux.HandleFunc("DELETE /api/campaigns/{id}", s.requireAuth(s.campaignScoped(s.gate("manage_campaigns", s.handleDeleteCampaign))))
	mux.HandleFunc("POST /api/campaigns/{id}/clone", s.requireAuth(s.campaignScoped(s.gate("manage_campaigns", s.handleCloneCampaign))))
	// Branches (sub-units of a campaign / branch-group).
	mux.HandleFunc("GET /api/campaigns/{id}/ga4", s.requireAuth(s.campaignScoped(s.handleCampaignGa4Report)))
	mux.HandleFunc("GET /api/ga4/report", s.requireAuth(s.handleOrgGa4Report))
	mux.HandleFunc("GET /api/campaigns/{id}/branches", s.requireAuth(s.campaignScoped(s.handleListBranches)))
	mux.HandleFunc("POST /api/campaigns/{id}/branches", s.requireAuth(s.campaignScoped(s.gate("manage_campaigns", s.handleCreateBranch))))
	mux.HandleFunc("PATCH /api/branches/{id}", s.requireAuth(s.branchScoped(s.gate("manage_campaigns", s.handleUpdateBranch))))
	...
	mux.HandleFunc("GET /api/campaigns/{id}/usage", s.requireAuth(s.campaignScoped(s.handleCampaignUsage)))
	mux.HandleFunc("GET /api/campaigns/{id}/usage.csv", s.requireAuth(s.campaignScoped(s.handleCampaignUsageCSV)))
	mux.HandleFunc("POST /api/campaigns/{id}/ai-style/suggest", s.requireAuth(s.campaignScoped(s.gate("manage_campaigns", s.handleSuggestAIStyle))))
	mux.HandleFunc("POST /api/campaigns/{id}/ai-style/preview", s.requireAuth(s.campaignScoped(s.gate("manage_campaigns", s.handlePreviewAIStyle))))
	// Segment-generic campaign catalog / KB (per-campaign pricing the bot grounds on).
	mux.HandleFunc("GET /api/campaigns/{id}/catalog", s.requireAuth(s.campaignScoped(s.handleListCatalog)))
	mux.HandleFunc("POST /api/campaigns/{id}/catalog", s.requireAuth(s.campaignScoped(s.gate("manage_campaigns", s.handleUploadCatalog))))
	mux.HandleFunc("POST /api/campaigns/{id}/catalog/extract", s.requireAuth(s.campaignScoped(s.gate("manage_campaigns", s.handleExtractCatalog))))
	mux.HandleFunc("GET /api/campaigns/{id}/catalog/extract/{job}", s.requireAuth(s.campaignScoped(s.gate("manage_campaigns", s.handleExtractCatalogStatus))))
	mux.HandleFunc("PATCH /api/campaigns/{id}/catalog/{row}", s.requireAuth(s.campaignScoped(s.gate("manage_campaigns", s.handleUpdateCatalogRow))))
	mux.HandleFunc("DELETE /api/campaigns/{id}/catalog/{row}", s.requireAuth(s.campaignScoped(s.gate("manage_campaigns", s.handleDeleteCatalogRow))))
	mux.HandleFunc("DELETE /api/campaigns/{id}/catalog", s.requireAuth(s.campaignScoped(s.gate("manage_campaigns", s.handleClearCatalog))))
	...
	mux.HandleFunc("GET /api/public/org-search", authRL(s.handlePublicOrgSearch))
			...
			for _, m := range val.Messages {
				...
				evt := events.MessageReceived{
					...
					Message: events.InboundMessage{
						...
						ReplyToExternalID: m.replyToWamid(),
					},
					Raw: raw,
				}
