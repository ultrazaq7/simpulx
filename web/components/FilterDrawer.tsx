      ...
      <div className="fixed right-0 top-0 h-full w-[320px] max-w-[90vw] bg-card border-l border-border shadow-2xl z-[60] flex flex-col animate-slide-in-right">
        ...
        <div className="flex items-center gap-2 px-4 py-3 border-t border-border shrink-0">
          <button onClick={onClear} disabled={!canClear}
            className="flex-1 h-9 rounded-md border border-border text-[13px] font-semibold text-foreground hover:bg-muted disabled:opacity-50 outline-none transition-colors">Clear</button>
          <button onClick={onClose}
            className="flex-1 h-9 rounded-md bg-primary text-white text-[13px] font-semibold hover:bg-primary-dark outline-none transition-colors">Apply</button>
        </div>
      </div>
    </>
  );
