...
export const api = {
  ...
  createWebApiSource: (input: { name: string; slug?: string; auto_template_name?: string; webhook_url?: string; campaign_id?: string; platform?: SourcePlatform }) =>
    req<{ id: string; api_key: string }>("/api/web-api-sources", { method: "POST", body: JSON.stringify(input) }),
  ...
  extractCatalogPdf: async (id: string, input: { pdf_base64: string; segment?: string; force?: boolean }, onProgress?: (info: { stage?: string; rows?: number }) => void) => {
    ...
    while (Date.now() < deadline) {
      ...
      onProgress?.({ stage: st.stage, rows: st.rows_found }); // pending -> report live progress
    }
  ...
  campaignAdsLive: (id: string) =>
    req<{ status: string; ads: { id: string; name: string; status: string; thumbnail: string; image: string; campaign_name: string; adset_name: string }[]; error?: string }>(
  ...
  revenue: (from?: string, to?: string, type?: string) => {
    const qs = new URLSearchParams();
    if (from) qs.set("from", from);
    if (to) qs.set("to", to);
  ...
  connectGoogleAds: (customer_id: string, name?: string) =>
  ...
  updateAdAccount: (id: string, patch: { name?: string; external_account_id?: string; access_token?: string; capi_dataset_id?: string; capi_access_token?: string; access_mode?: string }) =>
