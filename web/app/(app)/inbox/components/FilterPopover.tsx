...
export default function FilterPopover({ categories, toggles, activeCount, onClearAll, onClose }: FilterPopoverProps) {
  ...
  // ── Shared: category list (left panel / first step) ──
  const categoryList = (
    <>
      {categories.map((c) => {
  ...
  // ── Shared: options panel for the active category ──
  ...
  return (
    <>
      {/* ── DESKTOP (lg+): side-by-side master-detail popover ── */}
      ...
      {/* ── MOBILE (< lg): fullscreen wizard overlay ── */}
        ...
        <div className="fixed inset-0 z-50 bg-background flex flex-col animate-scale-in">
          ...
          <div className="shrink-0 h-14 px-4 flex items-center gap-3 border-b border-border">
            {activeCat ? (
              <button
                type="button"
                onClick={() => { setActiveKey(""); setSearch(""); }}
                className="p-1 -ml-1 rounded-md text-muted-foreground hover:bg-muted hover:text-foreground outline-none"
              >
            ...
            <button
              type="button"
              onClick={onClose}
              className="p-1 rounded-md text-muted-foreground hover:bg-muted hover:text-foreground outline-none"
            >
              <X className="w-5 h-5" />
            </button>
          </div>
          ...
          <div className="flex-1 overflow-y-auto">
            {activeCat ? (
              /* Step 2: options for selected category */
              <div className="flex flex-col h-full">
                {optionsPanel}
              </div>
            ) : (
              /* Step 1: category list + toggles */
              <div className="py-1">
                {categoryList}
              </div>
            )}
