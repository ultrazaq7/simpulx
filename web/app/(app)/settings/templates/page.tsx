...
import { Search, Plus, Pencil, Trash2, Send, RefreshCw, Loader2, X } from "lucide-react";
...
export default function TemplatesPage() {
  ...
  async function load() { setLoading(true); try { setRows(await api.listTemplates()); } finally { setLoading(false); } }
  useEffect(() => { load(); }, []);
  ...
  return (
    <PageBody>
      ...
      <div className="flex items-center gap-3 mb-5 flex-wrap">
        ...
        <Select value={statusFilter} onChange={setStatusFilter} placeholder="All statuses" className="min-w-[150px]"
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-border bg-muted/40">
              {["Name", "Category", "Language", "Status", "Updated", "Actions"].map((h) => (
          ...
          <tbody>
            {loading ? [0, 1, 2].map((i) => (
              <tr key={i}><td colSpan={6} className="px-4 py-3"><div className="h-7 rounded-md skeleton" /></td></tr>
            )) : filtered.length === 0 ? (
...
function TemplateDialog({ open, editing, onClose, onSaved, onError }: {
  ...
  async function save() {
    ...
    setSaving(true);
    const payload = { name: name.trim(), category, language, header_type: headerType, header_text: headerType === "TEXT" ? headerText : "", body, footer, buttons, variables: vars };
    try {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
      ...
      <div className="relative bg-card rounded-lg border border-border shadow-2xl w-full max-w-3xl max-h-[90vh] flex flex-col animate-scale-in">
          ...
          <div className="w-[300px] shrink-0">
            ...
            <div className="bg-[#E5DDD5] rounded-lg p-4 min-h-[200px]" style={{ backgroundImage: "radial-gradient(rgba(0,0,0,0.04) 1px,transparent 1px)", backgroundSize: "16px 16px" }}>
              <div className="bg-white rounded-lg rounded-tl-none p-2.5 shadow-sm max-w-[250px]">
                {headerType === "TEXT" && headerText && <p className="text-[13.5px] font-bold mb-1">{headerText}</p>}
              ...
              {buttons.length > 0 && (
                <div className="mt-1.5 flex flex-col gap-1 max-w-[250px]">
                  {buttons.map((b, i) => (
                    <div key={i} className="bg-white rounded-lg py-1.5 text-center text-[#1DA1F2] font-semibold text-[13px] shadow-sm">{b.text || "Button"}</div>
                  ))}
                </div>
              )}
            </div>
