...
function LeadFunnel({ stages, lost = 0 }: { stages?: Analytics["funnel_stages"]; lost?: number }) {
  if (!stages || stages.length === 0) {
    return <div className="py-10 text-center text-sm text-muted-foreground">No pipeline data yet</div>;
  }
  // Lost leads all entered the funnel, so fold them into the entry stage's cumulative
  // count. They aren't added to later stages since we can't know how far each got.
  const rows = stages.map((s, i) => (i === 0 ? { ...s, reached: s.reached + lost } : s));
  const top = Math.max(rows[0]?.reached ?? 0, 1);
  return (
    <div className="p-4 space-y-2.5">
      {rows.map((s, i) => {
        const pct = (s.reached / top) * 100;
        const conv = i === 0 ? null : (rows[i - 1].reached > 0 ? (s.reached / rows[i - 1].reached) * 100 : 0);
        return (
      ...
      <div className="p-4">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 mb-5">
          ...
          <Card title="Lead funnel" subtitle="Reached each stage and beyond">
            <LeadFunnel stages={analytics?.funnel_stages} lost={analytics?.funnel?.lost} />
          </Card>
